...
def asana_chat(app, mongo, cache, refresh_functions, query=None):
    ...
    1. **Clasificación del Tipo de Solicitud**:
       ...
       - **Solicitud GET**: Si el usuario pide información con verbos como 'Mándame', 'Pásame', 'Envíame', 'Muéstrame', 'Busca', 'Dame', 'Dime', 'Quiero ver', 'Lista', 'Encuentra' (ej. 'Dame las tareas de Proyecto X'), responde con: `"Es una solicitud GET"`.
    ...
    2. **Reglas Críticas para Clasificación**:
       - **GET**: Solicitudes de lectura solo para Asana (obtener tareas, proyectos, secciones).
       - **POST**: Acciones de escritura solo para Asana (crear tareas, actualizar tareas, eliminar tareas).
       - **Automatizadas**: Acciones con condiciones, detectando intenciones para Asana y otras APIs mencionadas por el usuario.
    ...
    3. **Detección y Generación de Consultas**:
       - Para **GET y POST simples**, genera intenciones solo para Asana:
         - **Asana**: Buscar tareas, obtener proyectos, crear tareas, actualizar tareas, eliminar tareas, asignar tareas.
    ...
    4. **Formato de Salida**:
       ...
       - **GET y POST simples**: Usa 'N/A' si no aplica a Asana.
       - **Automatizadas**: Lista condiciones y acciones, incluyendo otras APIs si se mencionan.
       - **Múltiples**: Lista todas las intenciones detectadas como un array, sin filtrar por Asana.
    ...
    6. **Reglas para Consultas Generales**:
       - **GET**: Describe qué obtener en Asana (ej. "obtener tareas de Proyecto X"). Si no aplica, "No Clasificable".
       - **POST**: Describe la acción en Asana (ej. "crear tarea Revisión"). Si no aplica, "No Clasificable".
    ...
    Ejemplos:
    - "Dame las tareas de Proyecto X" → "Es una solicitud GET" {{"asana": "obtener tareas de Proyecto X"}}
    - "Crear tarea Revisión" → "Es una solicitud POST" {{"asana": "crear tarea Revisión"}}
    - "Si creo una tarea, asigna a Juan" → "Es una solicitud automatizada" {{"asana": [{{"condition": "crear una tarea", "action": "asignar a Juan"}}]}}
    - "Busca tareas y crea una nueva" → "Es una solicitud múltiple" {{"asana": ["obtener tareas", "crear una nueva tarea"]}}
    - "Hola" → "Es un saludo" {{"asana": "N/A"}}
    - "Sube un archivo a Drive" → "No puedo clasificar la solicitud, por favor aclara qué quieres hacer" {{"message": "Esto no es una acción para Asana, ¿qué quieres hacer con Asana?"}}
    """
    ...
    def handle_get_request(intencion, email):
        ...
        if not asana_token:
            return {"solicitud": "GET", "result": {"error": "¡Ey! No tengo tu token de Asana, ¿me das permisos? 🔑"}}, 400

        headers = {'Authorization': f"Bearer {asana_token}", 'Content-Type': 'application/json'}
        url = "https://app.asana.com/api/1.0/projects/some_project_id/tasks"  # Project ID debería ser dinámico

        query = intencion["asana"]
        if not query or query == "N/A":
            return {"solicitud": "GET", "result": {"error": "¡Falta algo, papu! ¿Qué quieres buscar en Asana? 🤔"}}, 400

        try:
            if "obtener tareas" in query.lower():
                project_name = query.split("de")[-1].strip() if "de" in query else ""
                # Simulación: en un entorno real, necesitarías el project_id correspondiente
                response = requests.get(url, headers=headers, params={"opt_fields": "name,permalink_url"})
                response.raise_for_status()
                tasks = response.json().get('data', [])
                results = [{"task_name": task["name"], "url": task["permalink_url"]} for task in tasks]
                if not results:
            ...
            return {"solicitud": "GET", "result": {"error": f"¡Ay, qué mala onda! Error con Asana: {str(e)}"}}, 500
    ...
    def handle_post_request(intencion, email):
        ...
        asana_token = user.get('integrations', {}).get('asana', {}).get('token')
        if not asana_token:
            return {"solicitud": "POST", "result": {"error": "¡Ey! No tengo tu token de Asana, ¿me das permisos? 🔑"}}, 400
        ...
        try:
            ...
            if "crear tarea" in query.lower():
                ...
                payload = {"data": {"name": task_name, "projects": ["some_project_id"]}}  # Project ID debería ser dinámico
            ...
            # Actualizar tarea
                ...
                search_url = "https://app.asana.com/api/1.0/tasks"
                response = requests.get(search_url, headers=headers, params={"opt_fields": "name,gid", "project": "some_project_id"})
                response.raise_for_status()
                tasks = response.json().get('data', [])
                task_id = next((t["gid"] for t in tasks if t["name"].lower() == task_name.lower()), None)
                if not task_id:
                    return {"solicitud": "POST", "result": {"message": f"📭 No encontré la tarea '{task_name}'"}}, 200
                url = f"https://app.asana.com/api/1.0/tasks/{task_id}"
                payload = {"data": {"name": f"{task_name} - {update_content}"}}
            ...
            return {"solicitud": "POST", "result": {"error": "¡Uy! Acción no soportada en Asana, ¿qué tal crear o actualizar una tarea? 😅"}}, 400

        except requests.RequestException as e:
            return {"solicitud": "POST", "result": {"error": f"¡Ay, qué mala onda! Error con Asana: {str(e)}"}}, 500
        except Exception as e:
    ...
    def chatAsana():
        ...
        if not user_query:
            return jsonify({"error": "¡Ey! Dame algo pa’ trabajar, ¿qué quieres hacer con Asana? 🤔"}), 400
        ...
        try:
            ...
            Si es un saludo (como "hola", "holaaaa"), responde: "Es un saludo" {{"asana": "N/A"}}
            ...
            if not request_type_match:
                ...
                if request_type == "Es un saludo":
                    greeting_prompt = f"El usuario dijo {user_query}. Responde de manera cálida y amigable con emojis a un saludo simple. Menciona que eres su asistente personalizado de Asana."
                    ...
                    result = {"solicitud": "ERROR", "result": {"error": parsed_response.get("message", "¡No entendí qué quieres hacer con Asana! 😕")}}
