...
from .getFunctionClickup import handle_get_request
from .postFunctionClickup import handle_post_request
from app.routes.core.context.ContextHandler import ContextHandler

def process_clickup_chat(email, user_query=None, mongo=None, cache=None, refresh_functions=None):
    """Core logic for processing ClickUp chat requests."""
    ...
    - **GET**: Solicitudes de lectura solo para ClickUp (obtener tareas, listas). Ejemplo: 'Dame las tareas en Proyectos' → GET.
    ...
    - **POST**: Acciones de escritura solo para ClickUp (crear tareas, eliminar tareas). Ejemplo: 'Crear tarea en Proyectos' → POST.
    - **Automatizadas**: Acciones con condiciones, detectando intenciones para ClickUp y otras APIs. Ejemplo: 'Si creo una tarea en Proyectos, envía un correo' → Automatizada.
    ...
    - Para **GET y POST simples**, genera intenciones solo para ClickUp:
        - **ClickUp**: Buscar tareas, obtener listas, crear tareas, actualizar tareas, eliminar tareas.
    ...
    - **GET**: Describe qué obtener en ClickUp (ej. "obtener tareas en Proyectos"). Si no aplica, clasifica como "No Clasificable".
    ...
    - **POST**: Describe la acción en ClickUp (ej. "crear tarea en Proyectos"). Si no aplica, clasifica como "No Clasificable".
    ...
    - Incluye nombres de tareas o listas clave del usuario (ej. "Reunión", "Proyectos") si se mencionan.
    ...
    - **Tareas o Listas Específicas**: Si se pide una tarea o lista específica (ej. 'la tarea Reunión'), inclúyelo en la intención (ej. "obtener la tarea Reunión").
    - **Contexto Implícito**: Si el usuario no especifica una tarea o lista en una solicitud GET_CONTEXT, asume que se refiere a la última tarea o lista mencionada en el historial (ej. 'Qué dice la tarea?' → "detalle de la última tarea mencionada").

    Ejemplos:
    - "Mandame las tareas en Proyectos" → "Es una solicitud GET" {{"clickup": "obtener tareas en Proyectos"}}
    - "Dame las tareas de Juan" → "Es una solicitud GET" {{"clickup": "obtener tareas de Juan"}}
    - "Qué dice la tarea Reunión?" → "Es una solicitud GET de contexto" {{"clickup": "detalle de la tarea Reunión"}}
    - "Crear tarea en Proyectos" → "Es una solicitud POST" {{"clickup": "crear tarea en Proyectos"}}
    - "Si creo una tarea en Proyectos, envía un correo" → "Es una solicitud automatizada" {{"clickup": [{{"condition": "crear una tarea en Proyectos", "action": "enviar un correo"}}]}}
    - "Busca tareas en Proyectos y crea una nueva" → "Es una solicitud múltiple" {{"clickup": ["obtener tareas en Proyectos", "crear tarea"]}}
    - "Hola" → "Es un saludo" {{"clickup": "N/A"}}
    - "Enviar mensaje a Slack" → "No puedo clasificar la solicitud, por favor aclara qué quieres hacer" {{"message": "Esto no es una acción para ClickUp, ¿qué quieres hacer con ClickUp?"}}
    """
    ...
    def generate_prompt(get_result):
        ...
        Eres un asistente de ClickUp súper amigable y útil, con un tono relajado y natural, como si charlaras con un amigo. Usa emojis sutiles para darle onda, pero sin exagerar. Basándote en esta info, arma una respuesta concisa y en párrafo que resuma los resultados de forma práctica y clara:
        ...
        try:
            response = openai.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "Eres un asistente de ClickUp amigable."},
    ...
    if "chats" not in user or not any(chat.get("name") == "ClickUpChat" for chat in user.get("chats", [])):
    ...
    clickup_chat = next(
        (chat for chat in usuario.get("chats", []) if isinstance(chat, dict) and chat.get("name") == "ClickUpChat"),
        None
    )
    ...
    try:
        ...
        if "saludo" in peticion.lower():
            greeting_prompt = f"El usuario dijo '{user_query}', responde de manera cálida y amigable con emojis. Menciona que eres su asistente personalizado de ClickUp."
...
def setup_clickup_chat(app, mongo, cache, refresh_functions):
    """Register ClickUp chat route."""
    ...
    def chatClickUp():
        ...
        result = process_clickup_chat(email, user_query, mongo, cache, refresh_functions)
        print(result)
